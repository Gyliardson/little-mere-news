...
export default async function RootLayout({ children, params }: Readonly<{ children: React.ReactNode; params: Promise<{ lang: string }> }>) {
  const { lang } = await params;
  if (!SUPPORTED_LANGUAGES.has(lang)) notFound();

  return (
    <html lang={lang} className={`${inter.variable} ${montserrat.variable} h-full antialiased`}>
      <body className="min-h-full flex flex-col bg-[var(--color-background)] text-[var(--color-foreground)]">
        <header className="border-b border-primary/20 bg-background-secondary p-4 sticky top-0 z-50 shadow-md">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <nav aria-label="Main Navigation"><LanguageSwitcher /></nav>
          </div>
        </header>
        ...
        <footer className="border-t border-primary/20 bg-background-secondary p-6 mt-auto">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-sm text-foreground-muted">© {new Date().getFullYear()} Little Mere News. All rights reserved.</p>
