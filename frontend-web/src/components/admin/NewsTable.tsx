...
const t = {
  pt: {
    ...
    unsafeSourceUrl: "URL de origem não navegável",
    close: "Fechar",
  ...
  en: {
    ...
    unsafeSourceUrl: "Source URL is not navigable",
    close: "Close",
    editTitle: "Edit News",
    save: "Save Changes",
    saving: "Saving...",
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-primary/10">
              {news.map((item) => (
                <tr key={item.id} className="hover:bg-primary/5 transition-colors">
                  ...
                  <td className="p-4 flex items-center justify-end gap-2">
                    <button type="button" onClick={() => setViewItem(item)} className="p-2 text-foreground-muted hover:text-accent hover:bg-accent/10 rounded-lg transition-colors" aria-label={`${labels.view}: ${item.title_en}`}>
                      <Eye className="w-4 h-4" aria-hidden="true" />
                    </button>
                    <button type="button" onClick={() => openEdit(item)} className="p-2 text-foreground-muted hover:text-primary hover:bg-primary/10 rounded-lg transition-colors" aria-label={`${labels.edit}: ${item.title_en}`}>
                      <Edit2 className="w-4 h-4" aria-hidden="true" />
                    </button>
                    <button type="button" onClick={() => { setDeleteItem(item); setFeedback(null); }} className="p-2 text-foreground-muted hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors" aria-label={`${labels.delete}: ${item.title_en}`}>
              ...
              {news.length === 0 && (
                <tr><td colSpan={5} className="p-8 text-center text-foreground-muted">{labels.empty}</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {viewItem && (
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-foreground-muted">{new Date(viewItem.published_at).toLocaleDateString(isPt ? "pt-BR" : "en-US", { day: "2-digit", month: "long", year: "numeric" })}</span>
            </div>
            ...
            <div className="flex items-center justify-between gap-4 pt-2 border-t border-primary/10">
              ...
              {safeHttpUrl(viewItem.source_url) ? (
                <a href={safeHttpUrl(viewItem.source_url) ?? undefined} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm font-medium text-accent hover:text-accent-secondary transition-colors">
                  {labels.originalLink}<ExternalLink className="w-3.5 h-3.5" aria-hidden="true" />
                </a>
              ) : (
                <span className="text-sm text-foreground-muted" data-source-url-status="unsafe">{labels.unsafeSourceUrl}</span>
              )}
            </div>
        ...
        {editItem && (
          <div className="p-6 space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => !loading && setEditItem(null)} aria-label={labels.close} className="p-2 hover:bg-primary/10 rounded-lg transition-colors"><X className="w-5 h-5 text-foreground-muted" aria-hidden="true" /></button>
            </div>
            {feedback && (
